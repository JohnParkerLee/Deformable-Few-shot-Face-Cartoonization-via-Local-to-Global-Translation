...
def define_G(input_nc, output_nc, k, nz, ngf, netG='unet_128', use_spectral_norm=False,norm='batch', nl='relu',
             use_dropout=False, use_attention = False, init_type='xavier', init_gain=0.02, gpu_ids=[], where_add='input', upsample='bilinear'):
    net = None
    ...
    if netG == 'landmarknet':
        ...
        content_nc = input_nc
        style_nc = input_nc
        net_content = LandmarkNet_content(content_nc, style_nc, output_nc, 7, ngf, norm_layer = norm_layer, nl_layer=nl_layer,
                          use_dropout = use_dropout, use_attention = use_attention, use_spectral_norm=use_spectral_norm, upsample = upsample)
        net_style = LandmarkNet_style(content_nc, style_nc, output_nc, 7, ngf, norm_layer = norm_layer, nl_layer=nl_layer,
                          use_dropout = use_dropout, use_attention = use_attention, use_spectral_norm=use_spectral_norm, upsample = upsample)
        return init_net(net_content, init_type, init_gain, gpu_ids), init_net(net_style, init_type, init_gain, gpu_ids),
...
class LandmarkNetBlock(nn.Module):
    def   __init__(self, input_cont, outer_nc, inner_nc,
        ...
        """
        super(LandmarkNetBlock, self).__init__()
        ...
        #     raise NotImplementedError(
        #         'padding [%s] is not implemented' % padding_type)
        downconv1 = [nn.Linear(input_cont, inner_nc)]
        ...
        if outermost:
            if wo_skip:
                # upconv1 = upsampleLayer(
                #     inner_nc, outer_nc, upsample=upsample, padding_type=padding_type,
                #     use_spectral_norm=use_spectral_norm)
                if use_spectral_norm:
                    upconv1 = [SpectralNorm(nn.Linear(inner_nc, outer_nc))]
                else:
                    upconv1 = [nn.Linear(inner_nc, outer_nc)]
            else:
                if use_spectral_norm:
                    upconv1 = [SpectralNorm(nn.Linear(inner_nc * 2, outer_nc))]
                else:
                    upconv1 = [nn.Linear(inner_nc * 2, outer_nc)]
            down1 = downconv1
            up1 = [uprelu1] + upconv1 + [nn.Sigmoid()]
            if use_attention:
                up1 += [attn_layer]
            # if norm_layer is not None:
            #     up1 += [norm_layer(outer_nc)]
            if use_dropout:
                up1 += [nn.Dropout(0.5)]
        elif innermost:
            if use_spectral_norm:
    ...
    def forward(self, content, style_feature):
        x1 = self.down1(content)
        if self.outermost:
            mid1 = self.submodule(x1, style_feature)
            fake_landmark = self.up1(mid1)
            return fake_landmark

        elif self.innermost:
            # mid = torch.cat([x1,style_feature], 1)
            # mid = alpha * x1 + (1-alpha) * style_feature
            mid = torch.cat([x1, style_feature], 2)
            content_feature = self.up1(mid)
            if self.wo_skip:
                return content_feature
            else:
                return torch.cat([content_feature, content], 2)
        else:
            mid1 = self.submodule(x1, style_feature)
            content_feature = self.up1(mid1)
            if self.wo_skip:
                return content_feature
            else:
                return torch.cat([content_feature, content], 2)

class StyleEncoderBlocks(nn.Module):
    def __init__(self,input_style, outer_nc,
                 submodule=None, outermost=False, innermost=False, use_spectral_norm=False,
                 norm_layer=None, nl_layer=None, use_dropout=False, use_attention=False,
                 upsample='basic', padding_type='zero', wo_skip=False):
        super(StyleEncoderBlocks, self).__init__()
        self.wo_skip = wo_skip
        self.outermost = outermost
        self.innermost = innermost
        # downconv = []
        ...
        #     raise NotImplementedError(
        #         'padding [%s] is not implemented' % padding_type)
        downconv = nn.Linear(input_style, outer_nc)
        downrelu = nn.LeakyReLU(0.2, True)
        if outermost:
            down = [downconv]
            if norm_layer is not None:
                down += [norm_layer(outer_nc)]
        elif innermost:
            down = [downrelu, downconv]
        else:
            down = [downrelu, downconv]
    ...
    def forward(self, style):
        ...
        if self.innermost:
            ...
            mid = self.submodule(x)
            return mid

class StyleEncoder(nn.Module):
    def __init__(self, input_style, output_nc, num_downs, ngf=64,
                 norm_layer=None, nl_layer=None, use_dropout=False,
                 use_attention=False, use_spectral_norm=False, upsample='basic', wo_skip=False):
        super(StyleEncoder, self).__init__()
        max_nchn = 8
        style_block = StyleEncoderBlocks(ngf * max_nchn,  ngf * max_nchn,
                                      use_spectral_norm=use_spectral_norm, innermost=True,
                                      norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, wo_skip=not wo_skip)
        for i in range(num_downs - 5):
            style_block = StyleEncoderBlocks(ngf * max_nchn,  ngf * max_nchn, style_block,
                                          norm_layer=norm_layer, nl_layer=nl_layer, use_dropout=use_dropout,
                                          use_spectral_norm=use_spectral_norm, upsample=upsample, wo_skip=not wo_skip)
        style_block = StyleEncoderBlocks(ngf * 4, ngf * max_nchn, style_block,
                                      use_attention=use_attention,
                                      use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip=not wo_skip)
        style_block = StyleEncoderBlocks(ngf * 3, ngf * 4, style_block, use_attention=use_attention,
                                      use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip=not wo_skip)
        # style_block = StyleEncoderBlocks(ngf, ngf * 2, style_block, use_attention=use_attention,
        #                                  use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
        #                                  nl_layer=nl_layer, upsample=upsample, wo_skip=not wo_skip)
        style_block = StyleEncoderBlocks(input_style, ngf*3, style_block,
                                      use_spectral_norm=use_spectral_norm, outermost=True, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip=not wo_skip)
        self.model = style_block
    def forward(self, style):
        return self.model(style)

class ContentEncoder(nn.Module):
    def __init__(self, input_content, output_nc, num_downs, ngf=64,
                 norm_layer=None, nl_layer=None, use_dropout=False,
                 use_attention=False, use_spectral_norm=False, upsample='basic', wo_skip=False):
        super(ContentEncoder, self).__init__()
        max_nchn = 8  # max channel factor
        # construct unet structure
        dual_block = LandmarkNetBlock(ngf * max_nchn, ngf * max_nchn, ngf * max_nchn,
                                      use_spectral_norm=use_spectral_norm, innermost=True,
                                      norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, wo_skip= wo_skip)
        for i in range(num_downs - 5):
            dual_block = LandmarkNetBlock(ngf * max_nchn, ngf * max_nchn, ngf * max_nchn, dual_block,
                                          norm_layer=norm_layer, nl_layer=nl_layer, use_dropout=use_dropout,
                                          use_spectral_norm=use_spectral_norm, upsample=upsample, wo_skip= wo_skip)
        dual_block = LandmarkNetBlock(ngf * 4,  ngf * 4, ngf * max_nchn, dual_block,
                                      use_attention=use_attention,
                                      use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip= wo_skip)
        dual_block = LandmarkNetBlock(ngf * 3,  ngf * 3, ngf * 4, dual_block, use_attention=use_attention,
                                      use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip= wo_skip)
        # dual_block = LandmarkNetBlock(ngf, ngf, ngf * 2, dual_block, use_attention=use_attention,
        #                               use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
        #                               nl_layer=nl_layer, upsample=upsample, wo_skip=wo_skip)
        dual_block = LandmarkNetBlock(input_content, output_nc, ngf*3, dual_block,
                                      use_spectral_norm=use_spectral_norm, outermost=True, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip= wo_skip)
        self.model = dual_block
    def forward(self, content, style_feature):
        return self.model(content, style_feature)

class LandmarkNet_content(nn.Module):
    def __init__(self, input_content, input_style, output_nc, num_downs, ngf=64,
                 norm_layer=None, nl_layer=None, use_dropout=False,
                 use_attention=False, use_spectral_norm=False, upsample='basic', wo_skip=False):
        super(LandmarkNet_content, self).__init__()
        self.content_encoder = ContentEncoder(input_content, output_nc, num_downs, ngf=64,
                 norm_layer=norm_layer, nl_layer=nl_layer, use_dropout=use_dropout,
                 use_attention=use_attention, use_spectral_norm=use_spectral_norm, upsample=upsample, wo_skip=wo_skip)

    def forward(self, content, style_feature):
        # style_feature = self.encodeStyle(style)
        fake_image = self.content_encoder(content, style_feature)
        return fake_image

    # def decode(self, content, style_feature):
    #     images = self.content_encoder(content, style_feature)
    #     return images

class LandmarkNet_style(nn.Module):
    def __init__(self, input_content, input_style, output_nc, num_downs, ngf=64,
                 norm_layer=None, nl_layer=None, use_dropout=False,
                 use_attention=False, use_spectral_norm=False, upsample='basic', wo_skip=True):
        super(LandmarkNet_style, self).__init__()
        self.style_encoder = StyleEncoder(input_style, output_nc, num_downs, ngf=64,
                 norm_layer=norm_layer, nl_layer=nl_layer, use_dropout=use_dropout,
                 use_attention=use_attention, use_spectral_norm=use_spectral_norm, upsample=upsample, wo_skip=wo_skip)
