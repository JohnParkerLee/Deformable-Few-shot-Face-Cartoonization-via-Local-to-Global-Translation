...
def define_G(input_nc, output_nc, k, nz, ngf, netG='unet_128', use_spectral_norm=False,norm='batch', nl='relu',
    ...
    if netG == 'landmarknet':
        ...
        content_nc = input_nc
        style_nc = input_nc
        net_content = LandmarkNet_content(content_nc, style_nc, output_nc, 8, ngf, norm_layer = norm_layer, nl_layer=nl_layer,
                          use_dropout = use_dropout, use_attention = use_attention, use_spectral_norm=use_spectral_norm, upsample = upsample)
        net_style = LandmarkNet_style(content_nc, style_nc, output_nc, 8, ngf, norm_layer = norm_layer, nl_layer=nl_layer,
                          use_dropout = use_dropout, use_attention = use_attention, use_spectral_norm=use_spectral_norm, upsample = upsample)
...
class LandmarkNetBlock(nn.Module):
    def __init__(self, input_cont, outer_nc, inner_nc,
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError(
                'padding [%s] is not implemented' % padding_type)
        downconv += [nn.Conv2d(input_cont, inner_nc, kernel_size = 3,
        ...
        if outermost:
            ...
            resBlock = []
            for i in range(3):
                resBlock += [ResnetBlock(input_cont, padding_type, norm_layer, use_dropout, False)]
            upconv = upsampleLayer(
                inner_nc * 2, outer_nc, upsample=upsample, padding_type=padding_type,
                use_spectral_norm=use_spectral_norm)
            down = resBlock + [downrelu] + downconv
            up = [uprelu] + upconv
            if norm_layer is not None:
                up += [norm_layer(outer_nc)]
            up += resBlock
    ...
    def forward(self, content, style_feature):
        x = self.down(content)
        if self.outermost:
            mid = self.submodule(x, style_feature)
            fake_landmark = self.up(mid)
            return fake_landmark

        elif self.innermost:
            mid = torch.cat([x, style_feature], 1)
            content_feature = self.up(mid)
            if self.wo_skip:
                return content_feature
            else:
                return torch.cat([content_feature, content], 1)
        else:
            mid = self.submodule(x, style_feature)
            content_feature = self.up(mid)
            if self.wo_skip:
...
class StyleEncoderBlocks(nn.Module):
    def __init__(self,input_style, outer_nc,
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError(
                'padding [%s] is not implemented' % padding_type)
        downconv = nn.Conv2d(input_style, outer_nc, kernel_size=3,
                              stride=2, padding=pad)
        downrelu = nn.LeakyReLU(0.2, True)
        # attn_layer = None
        # if use_attention:
...
class StyleEncoder(nn.Module):
    def __init__(self, input_style, output_nc, num_downs, ngf=64,
        ...
        style_block = StyleEncoderBlocks(ngf * 2, ngf * 4, style_block, use_attention=use_attention,
                                      use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip=not wo_skip)
        style_block = StyleEncoderBlocks(ngf, ngf * 2, style_block, use_attention=use_attention,
                                      use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip=not wo_skip)
        style_block = StyleEncoderBlocks(input_style, ngf, style_block,
...
class ContentEncoder(nn.Module):
    def __init__(self, input_content, output_nc, num_downs, ngf=64,
        ...
        dual_block = LandmarkNetBlock(ngf * 2,  ngf * 2, ngf * 4, dual_block, use_attention=use_attention,
                                      use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip= wo_skip)
        dual_block = LandmarkNetBlock(ngf, ngf, ngf * 2, dual_block, use_attention=use_attention,
                                      use_spectral_norm=use_spectral_norm, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip= wo_skip)
        dual_block = LandmarkNetBlock(input_content, output_nc, ngf, dual_block,
                                      use_spectral_norm=use_spectral_norm, outermost=True, norm_layer=norm_layer,
                                      nl_layer=nl_layer, upsample=upsample, wo_skip= wo_skip)
...
class LandmarkNet_style(nn.Module):
    def __init__(self, input_content, input_style, output_nc, num_downs, ngf=64,
        ...
        self.style_encoder = StyleEncoder(input_style, output_nc, num_downs, ngf=64,
                 norm_layer=get_norm_layer(norm_type='instance'), nl_layer=nl_layer, use_dropout=use_dropout,
                 use_attention=use_attention, use_spectral_norm=use_spectral_norm, upsample=upsample, wo_skip=wo_skip)
    def forward(self,  style):
        style_feature = self.encodeStyle(style)
        # fake_image = self.content_encoder(content, style_feature)
