...
from data.image_folder import make_type_dataset, make_txt_dataset,make_json_dataset
...
class FinetuneDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        data_B_json = ToTensor()(self.load_json_data(np.random.choice(self.style_data, 1)[0]))
        # k_shot_data = np.random.choice(self.style_data, self.k, replace=False)
        data_A_style_set = np.random.choice(self.content_data, self.k, replace=False)
        data_B_style_set = np.random.choice(self.style_data, self.k, replace=False)
        data_A = {}
        data_B = {}
        data_B_style = {}
        data_A_style = {}
        for items in self.component:
            ...
            for k_i_data_A in data_A_style_set:
                data_A_temp = ToTensor()(self.load_json_data(k_i_data_A))
        ...
        return {'data_A': data_A, 'data_B':data_B,'data_B_style': data_B_style,'data_A_style':data_A_style}
