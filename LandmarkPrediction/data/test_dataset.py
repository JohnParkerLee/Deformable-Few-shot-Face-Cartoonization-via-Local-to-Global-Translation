...
class TestDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.content_data_paths = sorted(make_txt_dataset(self.content_data_dir))
        self.shape = opt.load_size
        self.part_class = opt.part_class
        self.len = len(self.style_data_paths)
        self.part = util.part
        # self.mode = mode
    ...
    def __getitem__(self, index):
        ...
        data_B = []
        np.random.seed()
        np.random.shuffle(self.style_data_paths)
        k_shot_data = np.random.choice(self.style_data_paths, self.k, replace=False)
        data_A_paths = self.content_data_paths[index%len(self.content_data_paths)]
        # data_A = ToTensor()(self.load_json_data(data_A))
        data_A = ToTensor()(np.loadtxt(data_A_paths))
        ...
        return {'data_A': data_A, 'data_B': torch.cat(data_B, dim=0).reshape(len(data_B), 1, -1), 'A_paths':data_A_paths}
    ...
    def load_json_data(self, data):
        ...
        self.labelJson.load(data)
        landmark = np.array(self.labelJson.shapes[0]['points'])
        points = []
    ...
    def __len__(self):
        ...
        return len(self.content_data_paths)
