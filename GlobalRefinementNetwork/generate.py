    ...
    with torch.no_grad():
        for i in range(len(g_list)):
            for j in range(args.n_sample):
                ...
                sample, _ = g_test([sample_z.data], truncation=args.truncation, truncation_latent=mean_latent, input_is_latent=False, randomize_noise=False)
                utils.save_image(
                    sample,
                    f'test_sample/'+str(j)+'.png',
                    nrow=1,
                    normalize=True,
                    range=(-1, 1),
                )
...
if __name__ == '__main__':
    ...
    if args.truncation < 1:
        ...
        mean_latent = g_target.module.mean_latent(int(1e2))
        # mean_latent = None
