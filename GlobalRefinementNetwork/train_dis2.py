        ...
        if which > 0:
            ...
            mean_w = generator.get_latent(torch.randn([inverted_style_code.size(0), 512]).to(device)).unsqueeze(
                1).repeat(1,
                          generator.n_latent,
                          1)
            id_swap = list(range(6, generator.n_latent))
            ...
            # if len(points_5_fake.shape)>2 and len(points_5_real.shape)>2 and points_5_fake.shape[0]==target_sampled_img.shape[0] and points_5_real.shape[0]==target_sampled_img.shape[0]:
                ...
                for idx, item in enumerate(points_5_fake):
                    if idx == 0:
                        ...
                        res = crop_img(target_sampled_img[idx], points_5_fake[idx].squeeze(0))
                        crop_fake = [torch.cat((itemA, itemB), 0) for (itemA, itemB) in zip(crop_fake, res)]
                        res = crop_img(source_sampled_img[idx], points_5_real[idx].squeeze(0))
                        crop_real = [torch.cat((itemA, itemB), 0) for (itemA, itemB) in zip(crop_real, res)]
                ...
                lpips_nose = lpips.forward_pair(local_nose, transforms.functional.resize(crop_fake[2], (80,96))).mean().to(
                    device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_netG_eye", type=str, default="/home/ss/work/cycle/checkpoints/fernand_combine/latest_net_G_Eyer.pth")
    parser.add_argument("--ckpt_netG_nose", type=str, default="/home/ss/work/cycle/checkpoints/fernand_combine/latest_net_G_Nose.pth")
    parser.add_argument("--ckpt_netG_mouth", type=str, default="/home/ss/work/cycle/checkpoints/fernand_combine/latest_net_G_Mouth.pth")
