...
class OriLandmarkGanModel(BaseModel):
    ...
    def __init__(self, opt, pretrain_G_content, pretrain_G_style,net_G='landmarknet', k=5, input_nc = 34, output_nc = 34, is_Train = False):
        ...
        #     self.visual_names.append(['reak_A2B', 'real_B2A'])
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>

        use_D = opt.isTrain and opt.lambda_GAN > 0.0
        self.model_names = ['G_content', 'G_style']
        self.netG_content, self.netG_style = networks.define_G(input_nc, output_nc, k, 8, 64,
                                                               netG=net_G,
                                                               norm='none', nl='relu', use_dropout=False,
                                                               init_type='normal',
                                                               init_gain=0.02,
                                                               gpu_ids=self.gpu_ids, where_add='all',
                                                               upsample='basic')
        if use_D:
            self.model_names += ['D']
        ...
        if opt.isTrain:
            ...
            # self.tripleLoss = torch.nn.TripletMarginWithDistanceLoss(distance_function=torch.nn.CosineSimilarity(dim=2), margin = 0.2)
            # self.styleLoss =
            ...
            load_style_path = pretrain_G_style
            load_content_path = pretrain_G_content
            style_state_dict = torch.load(load_style_path, map_location=str(self.device))
            if isinstance(self.netG_style, torch.nn.DataParallel):
                self.netG_style = self.netG_style.module
            self.netG_style.load_state_dict(style_state_dict, strict=True)
            content_state_dict = torch.load(load_content_path, map_location=str(self.device))
            if isinstance(self.netG_content, torch.nn.DataParallel):
                self.netG_content = self.netG_content.module
            self.netG_content.load_state_dict(content_state_dict, strict=True)
            self.netG_style.eval().cpu()
            self.netG_content.eval().cpu()
            self.set_requires_grad(self.netG_style, False)
            self.set_requires_grad(self.netG_content, False)
    ...
    def set_input(self, input):
        ...
        """
        # self.real_A = input['data_A'].float().to(self.device)
        # self.real_B = input['data_B'].float().to(self.device)
        # if not self.opt.finetune:
        #     self.real_A2B = input['data_AB'].float().to(self.device)
        # self.real_A_style = input['data_A_style'].float().to(self.device)
        ...
        # print(self.real_A_style.shape)
        # self.image_paths = input['A_paths' if AtoB else 'B_paths']

    def forward_data(self, real_A, real_B):
        # self.opt.k = k
        # B_feature = self.netG_style(
        #     real_B.reshape(real_B.shape[0] * self.opt.k, *real_A.shape[1:])).reshape(
        #     real_B.shape[0], self.opt.k, 1, 512)
        # B_feature = torch.mean(B_feature, 1)
        B_feature = torch.mean(self.netG_style(real_B), dim=1)
        fake_A2B = self.netG_content(real_A, B_feature)  # G(A)
        # Cycle
        # self.A_feature = self.netG_style(self.real_A_style)
        # self.fake_B2A = self.netG_content(self.fake_A2B, self.A_feature)
        return fake_A2B

    def forward(self):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        # self.fake_A2B = self.netG(self.real_A, self.real_B)  # G(A)
        ...
        # self.B_feature = self.netG_style(self.real_B.reshape(self.real_B.shape[0]*self.opt.k,*self.real_A.shape[1:])).reshape(self.real_B.shape[0], self.opt.k,1 ,512)
        # self.B_feature = self.netG_style(self.real_B)
        ...
        # # Cycle
        # self.A_feature = self.netG_style(self.real_A_style)
        # self.fakeB_feature = self.netG_style(self.real_A_style)
        # self.fake_B2A = self.netG_content(self.fake_A2B, self.A_feature)
        # self.fake_B2A_feature = self.netG_style(self.fake_B2A)
        # with torch.no_grad():
        #     self.A_feature = self.netG_style(self.real_A)
        #     self.A2B_feature= self.netG_style(self.fake_A2B)
